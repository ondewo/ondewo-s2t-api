...
IMAGE_UTILS_NAME=ondewo-s2t-api-utils:${ONDEWO_S2T_API_VERSION}
.DEFAULT_GOAL := help
...
install_python_requirements: ## Installs python requirements flak8 and mypy
	wget -q https://raw.githubusercontent.com/ondewo/ondewo-s2t-client-python/master/requirements.txt -O requirements.txt
	wget -q https://raw.githubusercontent.com/ondewo/ondewo-s2t-client-python/master/requirements-dev.txt -O requirements-dev.txt
	wget -q https://raw.githubusercontent.com/ondewo/ondewo-s2t-client-python/master/.flake8 -O .flake8
	wget -q https://raw.githubusercontent.com/ondewo/ondewo-s2t-client-python/master/mypy.ini -O mypy.ini
...
githubio_logic_pre:
	...
	$(eval REPO_NAME_UPPER:= $(shell echo ${GH_REPO} | cut -d "-" -f 2 | perl -pe 'tr/a-z/A-Z/'))
	$(eval DOCS_DIR:=ondewo.github.io/docs/ondewo-${REPO_NAME}-api/${ONDEWO_S2T_API_VERSION})
	@perl -i -ne "print unless /\{ number: '${ONDEWO_S2T_API_VERSION}', link: 'ondewo-${REPO_NAME}-api\/${ONDEWO_S2T_API_VERSION}\/' \},/" ondewo.github.io/data.js
	@rm -rf ${DOCS_DIR}
	@mkdir "${DOCS_DIR}"
	@cp docs/* ${DOCS_DIR}
	@perl -i -pe "s/h1>Protocol Documentation/h1>${REPO_NAME_UPPER} ${ONDEWO_S2T_API_VERSION} Documentation/" ${DOCS_DIR}/index.html

githubio_logic: | githubio_logic_pre
	$(eval REPO_NAME:= $(shell echo ${GH_REPO} | cut -d "-" -f 2 ))
	$(eval REPO_NAME_UPPER:= $(shell echo ${GH_REPO} | cut -d "-" -f 2 | perl -pe 'tr/a-z/A-Z/'))
	@git branch | grep "*" | grep -q "master" || (echo "Not on master branch"  & rm -rf ondewo.github.io && exit 1)
	@! cat ondewo.github.io/data.js | perl -ne "print if /name\: '${REPO_NAME_UPPER}'/../end\: ''/" | grep -q "number: '${ONDEWO_S2T_API_VERSION}'" || (echo "Already Released" && exit 1)
	...
	@export TEMP_TEXT="$$(perl -pe 's/VERSION/${ONDEWO_S2T_API_VERSION}/g; s/TECHNOLOGY/${REPO_NAME}/g' ondewo.github.io/script_object.txt)"; \
		perl -i -pe 'print "$$ENV{TEMP_TEXT}\n" if $$. == ${VERSION_LINE}' ondewo.github.io/data.js
	@npm install prettier && cd ondewo.github.io && npx prettier -w --single-quote data.js
	$(eval DOCS_DIR:=ondewo.github.io/docs/ondewo-${REPO_NAME}-api/${ONDEWO_S2T_API_VERSION})
	$(eval HEADER_LINE:= $(shell cat ${DOCS_DIR}/index.html | grep -n "${REPO_NAME_UPPER} ${ONDEWO_S2T_API_VERSION} Documentation" | grep -o -E '[0-9]+' | head -1 | perl -pe 's/^0+//'))
	$(eval TEMP_CALC:= $(shell expr ${HEADER_LINE} ))
	@export TEMP_IMG="$$(cat ondewo.github.io/script_image.txt)"; \
		perl -i -pe 'print "$$ENV{TEMP_IMG}\n" if $$. == ${TEMP_CALC}' ${DOCS_DIR}/index.html
	...
	@git -C ondewo.github.io status
	@git -C ondewo.github.io commit -m "Added ${REPO_NAME} Documentation for ${ONDEWO_S2T_API_VERSION}"
	@git -C ondewo.github.io push

update_githubio:
	@if [ -d "ondewo.github.io" ]; then \
		echo "Removing existing directory ondewo.github.io"; \
		rm -rf ondewo.github.io; sleep 3; \
	fi
...
create_release_branch: ## Create Release Branch and push it to origin
	git checkout -b "release/${ONDEWO_S2T_API_VERSION}"
	git push -u origin "release/${ONDEWO_S2T_API_VERSION}"

create_release_tag: ## Create Release Tag and push it to origin
	git tag -a ${ONDEWO_S2T_API_VERSION} -m "release/${ONDEWO_S2T_API_VERSION}"
	git push origin ${ONDEWO_S2T_API_VERSION}

login_to_gh: ## Login to Github CLI with Access Token
	@echo $(GITHUB_GH_TOKEN) | gh auth login -p ssh --with-token

build_gh_release: ## Generate Github Release with CLI
	gh release create --repo $(GH_REPO) "$(ONDEWO_S2T_API_VERSION)" -n "$(CURRENT_RELEASE_NOTES)" -t "Release ${ONDEWO_S2T_API_VERSION}"

delete_gh_release: ## Delete GitHub Release, release branch and release tag via gh CLI
	-gh release delete --repo $(GH_REPO) "$(ONDEWO_S2T_API_VERSION)" --yes
	-gh api repos/ondewo/ondewo-s2t-api/git/refs/heads/release/${ONDEWO_S2T_API_VERSION} -X DELETE
	-gh api repos/ondewo/ondewo-s2t-api/git/refs/tags/${ONDEWO_S2T_API_VERSION} -X DELETE
...
unrelease: build_utils_docker_image unrelease_to_github_via_docker_image ## Undo a release: delete the GitHub release, release branch, and release tag
	-git branch -d "release/${ONDEWO_S2T_API_VERSION}"
	-git tag -d "${ONDEWO_S2T_API_VERSION}"
	-git fetch --prune
	@echo "Unrelease of ${ONDEWO_S2T_API_VERSION} complete"

release_all_clients: ## Release all Clients of S2T
	@rm -f .already_released_marker; make release_python_client || { [ -f .already_released_marker ] && echo "SKIP: Python client already released ${ONDEWO_S2T_API_VERSION}" || { echo "FAILED: Python client did not release - aborting"; exit 1; }; }
	@rm -f .already_released_marker; make release_nodejs_client || { [ -f .already_released_marker ] && echo "SKIP: Nodejs client already released ${ONDEWO_S2T_API_VERSION}" || { echo "FAILED: Nodejs client did not release - aborting"; exit 1; }; }
	@rm -f .already_released_marker; make release_typescript_client || { [ -f .already_released_marker ] && echo "SKIP: Typescript client already released ${ONDEWO_S2T_API_VERSION}" || { echo "FAILED: Typescript client did not release - aborting"; exit 1; }; }
	@rm -f .already_released_marker; make release_angular_client || { [ -f .already_released_marker ] && echo "SKIP: Angular client already released ${ONDEWO_S2T_API_VERSION}" || { echo "FAILED: Angular client did not release - aborting"; exit 1; }; }
	@rm -f .already_released_marker; make release_js_client || { [ -f .already_released_marker ] && echo "SKIP: JS client already released ${ONDEWO_S2T_API_VERSION}" || { echo "FAILED: JS client did not release - aborting"; exit 1; }; }
...
release_client:
	...
	$(eval REPO_DIR:= $(shell echo "ondewo-s2t-client-${REPO_NAME}"))
...
# Change Version Number and RELEASE NOTES
	cd ${REPO_DIR} && perl -i -pe 'BEGIN{open F,"<","../temp-notes" or die; local $$/; $$c=<F>; close F} $$_ .= $$c if /Release History/' ${RELEASEMD}
...
NODEJS_CLIENT="git@github.com:ondewo/ondewo-s2t-client-nodejs.git"
...
TYPESCRIPT_CLIENT="git@github.com:ondewo/ondewo-s2t-client-typescript.git"
...
ANGULAR_CLIENT="git@github.com:ondewo/ondewo-s2t-client-angular.git"
...
JS_CLIENT="git@github.com:ondewo/ondewo-s2t-client-js.git"
...
spc: ## Checks if the Release Branch, Tag and Pypi version already exist
	$(eval filtered_branches:= $(shell git branch --all | grep "release/${ONDEWO_S2T_API_VERSION}"))
	$(eval filtered_tags:= $(shell git tag --list | grep "${ONDEWO_S2T_API_VERSION}"))
