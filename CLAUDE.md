...
```text
Release ONDEWO S2T API ${ONDEWO_S2T_API_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO S2T API <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO S2T API ' RELEASE.md     # must be >= 1 for your new version
```
